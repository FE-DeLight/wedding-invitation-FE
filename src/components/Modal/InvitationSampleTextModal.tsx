...
export default function InvitationSampleTextModal() {
  ...
  const { text, openModal } = useSelector((state: any) => state.invitation); // Redux 스토어의 상태 가져오기
  ...
  return (
    openModal && (
        ...
        >
          <DialogTitle id="scroll-dialog-title">샘플 텍스트
            <IconButton onClick={showSampleText}>
                <CloseIcon sx={{ fontSize: 18 }} color="disabled" />
            </IconButton>
          </DialogTitle>
